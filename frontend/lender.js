    ...
    loans.forEach(async (loan) => {
      if (loan.proposalId == proposalId) {
        const borrower = await App.contract.methods
          .proposalToBorrower(loan.proposalId)
          .call();
        ...
        await web3.eth
          .sendTransaction({
            from: App.account,
            to: borrower,
            value: web3.utils.toWei(loan.loanAmount, "ether"),
          })
...
// Handle mortgage modification form submission
$("#modifyMortgageForm").on("submit", async function (e) {
  e.preventDefault();

  const mortgageAddress = $("#mortgageAddress").val();
  const newValue = web3.utils.toWei($("#newValue").val(), "ether");
  const newStatus = $("#newStatus").val() === "true";
  const newLiens = $("#newLiens").val() === "true";
  const newInsurance = $("#newInsurance").val() === "true";

  try {
    const mortgageContract = new web3.eth.Contract(
      propertyABI,
      mortgageAddress
    );

    // Update property value
    await mortgageContract.methods
    ...
    await mortgageContract.methods
      .updateAdditionalInfo(newLiens, newInsurance)
      .send({ from: App.account });

    alert("Mortgage details updated successfully!");

    // Clear form
    $("#modifyMortgageForm")[0].reset();
  } catch (error) {
    console.error("Error updating mortgage:", error);
    alert(
      "Error updating mortgage details. Please check the address and try again."
    );
  }
});
